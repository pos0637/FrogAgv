<template>
  <div class="app-container">
    <el-table :data="data" 
        ...
        :height="tableHeight" :row-dblclick="rowDblclick">
      <el-table-column v-for="(column, index) in columns" :key="column.value" :label="tableTitle(column.text)">
    ...
    <div class="pagination-container" v-if="showPage === true">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="fetchParams.pageNum || pageNum" :page-sizes="[10,20,30, 50]" :page-size="fetchParams.pageSize || pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList(_fetchParams, _fetchUrl) {
      ...
      if (((isEmpty(this.ascs)) || (this.ascs.length === 0)) && ((isEmpty(this.descs)) || (this.descs.length === 0))) {
    ...
    renderSelect(val) {
      ...
      if (this.multipleTable && !isEmpty(val) && (!isEmpty(this.data)) && (this.data instanceof Array)) {
