<template>
  <div style="position:relative">
    <el-select v-model="values" placeholder="请选择" ref="refreshSelect" v-bind="$attrs" style="width:100%" :disabled="disabled">
      <el-option v-for="item in options" :key="item.value + item.label" :label="item.label" :value="item.value">
      </el-option>
    </el-select>
    <span style="position:absolute;top:0px;right:30px;line-height:36px;" :class="disabled ? 'hidden' : 'bb' ">
      <i class='el-icon-refresh pointer' @click="handleIconClick" style="background-color: #ffffff;"></i>
    </span>
  </div>
</template>
